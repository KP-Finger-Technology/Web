...
if($_GET) {
	if(isset($_POST['btnSave'])){
		...
		if(count($message)==0){			
			$qryUpdate=mysql_query("UPDATE services SET tittle='$txttittle', content='$txtcontent' WHERE serviceid ='".$txtKode."'")or die ("Gagal query".mysql_error());
			if($qryUpdate){
	...
	$sqlShow = "SELECT * FROM services WHERE serviceid='$KodeEdit'";
	$qryShow = mysql_query($sqlShow, $koneksi) or die ("Query ambil data services salah:".mysql_error());
	$dataShow= mysql_fetch_array($qryShow);	
								
	# MASUKKAN DATA KE VARIABEL
	$dataKode	= $dataShow['serviceid'];
                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                <div class="box-header">
                                    <h3 class="box-title">Mobile Apps Content Data Table</h3>                                    
                                </div><!-- /.box-header -->
                                <div class="box-body">
                                    <form role="form" action="?page=mobile_edit" method="post" name="frmedit" target="_self" id="frmedit" enctype="multipart/form-data">
                                        <!-- text input -->
